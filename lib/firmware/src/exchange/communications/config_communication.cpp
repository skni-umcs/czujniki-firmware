...
std::shared_ptr<ConfigCommunication> ConfigCommunication::create(
    std::shared_ptr<ConfigurationFacade> configFacade) {
  auto configCommunication =
      std::shared_ptr<ConfigCommunication>(new ConfigCommunication());
...
OperationResult ConfigCommunication::getNotified(
  ...
  // No current destination check - configs are global and broadcasted unless
  // specified otherwise in the updateConfig switch case
...
bool isBroadcastOrCurrentDestination(char reach, moduleAddress destination) {
  return reach == '*' ||
         destination == AddressHandler::getInstance()->readAddress();
}

OperationResult ConfigCommunication::updateConfig(
    MessageContent messageContent, std::shared_ptr<Message> message) {
  ...
  char reach = static_cast<char>(
      std::tolower(static_cast<unsigned char>(configDetails[0])));
  char cmd = static_cast<char>(
      std::tolower(static_cast<unsigned char>(configDetails[1])));
  ...
  try {
    if (isBroadcastOrCurrentDestination(reach, destination)) {
      switch (cmd) {
        ...
        case 'r': {
          if (!preferences.isKey(LAST_REMOTE_RESET_TIMESTAMP) ||
              preferences.getUShort(LAST_REMOTE_RESET_TIMESTAMP) !=
                  messageContent.getJsonificationEpoch()) {
            preferences.putUShort(LAST_REMOTE_RESET_TIMESTAMP,
                                  messageContent.getJsonificationEpoch());
            ESP.restart();
          }
          return OperationResult::SUCCESS;
