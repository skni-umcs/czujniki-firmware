...
std::shared_ptr<ServiceCommunication> ServiceCommunication::create() {
  auto serviceCommunication =
      std::shared_ptr<ServiceCommunication>(new ServiceCommunication());

  serviceCommunication->timeSyncTimer.get()->setExecuteFunction(
      [serviceCommunication]() { serviceCommunication->timeSync(); });
  serviceCommunication->timeSyncTimer.get()->updateTime(
      serviceCommunication->timeSyncPeriodMs);

  return serviceCommunication;
}

OperationResult ServiceCommunication::getNotified(
  ...
  MessageContent serverMessage =
      MessageContent::fromJson(message->getContent());
...
void ServiceCommunication::sendResetReason() {
  ...
  int reset_reasons[] = {rtc_get_reset_reason(0), rtc_get_reset_reason(1)};
  for (const auto& reason : reset_reasons) {
    messages.add(reason);
  }
  serializeJson(doc, serializedJson);
  MessageContent packetMessage =
      MessageContent(TransmissionCode::RESET, serializedJson);
...
OperationResult ServiceCommunication::timeSync() {
  askTimeTimeoutTimer.get()->setExecuteFunction([this]() { askForTime(); });
...
OperationResult ServiceCommunication::askForTime() {
  ...
  MessageContent packetMessage =
      MessageContent(TransmissionCode::TIME_SYNCHRONIZATION, "t");
...
OperationResult ServiceCommunication::updateTime(unsigned long serverTime) {
  ...
  // Powiadom TimeSlotManager o synchronizacji czasu
  if (timeSlotManager) {
    timeSlotManager->markTimeSynchronized(serverTime + RTT / 2);
    Logger::logf("TimeSlotManager synchronized - module will use slot %d\n",
...
unsigned long ServiceCommunication::getLastAskTime() { return lastAskTime; }

OperationResult ServiceCommunication::setLastAskTime(
    unsigned long lastAskTime) {
...
OperationResult ServiceCommunication::setTimeSyncPeriodMs(
    int timeSyncPeriodMs) {
  this->timeSyncPeriodMs = timeSyncPeriodMs;
  timeSyncTimer.get()->updateTime(timeSyncPeriodMs);
  return OperationResult::SUCCESS;
}

int ServiceCommunication::getAskTimeoutMs() { return askTimeoutMs; }

int ServiceCommunication::getTimeSyncPeriodMs() { return timeSyncPeriodMs; }

void ServiceCommunication::setTimeSlotManager(
    std::shared_ptr<TimeSlotManager> manager) {
  timeSlotManager = manager;
  Logger::log("TimeSlotManager set in ServiceCommunication");
}
