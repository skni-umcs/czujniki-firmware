...
class Message {
 ...
 public:
  ...
  Message(std::vector<moduleAddress> senders, moduleAddress destination,
          std::string content, std::vector<std::string> rssi,
          unsigned char hopLimit);
...
class LoraMessage : public Message {
 ...
 public:
  ...
  LoraMessage(std::vector<moduleAddress> senders, moduleAddress destination,
              std::string content, std::vector<std::string> rssi,
              unsigned char hopLimit, byte currentRssiByte, int snr);
  std::string createOwnAddressTable() override;
  int getSnr();
  byte getCurrentRssiByte();
  MessageType type() const override { return MessageType::LoraMessage; }
};

class GeneratedMessage : public Message {
 public:
  GeneratedMessage(std::vector<moduleAddress> senders,
                   moduleAddress destination, std::string content,
                   std::vector<std::string> rssi, unsigned char hopLimit);
  static std::shared_ptr<GeneratedMessage> fromText(std::string text,
                                                    moduleAddress destination);
  MessageType type() const override { return MessageType::GeneratedMessage; }
};
...
class TextMessage : public Message {
 public:
  ...
  MessageType type() const override { return MessageType::TextMessage; }
};
